...
SET
    ...
    error_detalle = CONCAT_WS(
        ...
        CASE WHEN raw_numero_tic IS NOT NULL
              ...
              AND TRIM(raw_numero_tic) !~ '^[0-9]+$'
            THEN 'raw_numero_tic no es entero' END,
        CASE WHEN raw_pos_x IS NULL OR TRIM(raw_pos_x) !~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN 'raw_pos_x invalido' END,
        CASE WHEN raw_pos_y IS NULL OR TRIM(raw_pos_y) !~ '^-?[0-9]+(\.[0-9]+)?$'
...
WITH sectores_nuevos AS (
    SELECT
        ...
        MIN(CASE WHEN TRIM(s.raw_pos_x) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_pos_x)::FLOAT ELSE 0 END) AS bbox_x_min,
        MAX(CASE WHEN TRIM(s.raw_pos_x) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_pos_x)::FLOAT ELSE 0 END) AS bbox_x_max,
        MIN(CASE WHEN TRIM(s.raw_pos_y) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_pos_y)::FLOAT ELSE 0 END) AS bbox_y_min,
        MAX(CASE WHEN TRIM(s.raw_pos_y) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_pos_y)::FLOAT ELSE 0 END) AS bbox_y_max
    FROM staging_telemetria s
    WHERE s.error_flag = FALSE
    GROUP BY TRIM(s.raw_sector)
    HAVING TRIM(s.raw_sector) NOT IN (SELECT nombre_sector FROM sectores)
),
numerados AS (
    SELECT
...
WITH tics_nuevos AS (
    SELECT DISTINCT
        ...
        CASE WHEN TRIM(s.raw_timestamp_ms) ~ '^[0-9]+$'
            THEN TRIM(s.raw_timestamp_ms)::INTEGER ELSE 0 END AS timestamp_ms,
        TRIM(s.raw_sesion)::INTEGER AS sesion_id
    FROM staging_telemetria s
    WHERE s.error_flag = FALSE
    AND NOT EXISTS (
        SELECT 1 FROM tic t
        WHERE t.numero_tic = TRIM(s.raw_numero_tic)::INTEGER
          AND t.sesiones_juegosesion_id = TRIM(s.raw_sesion)::INTEGER
    )
),
numerados AS (
    SELECT
        ...
        numero_tic, timestamp_ms, sesion_id
    FROM tics_nuevos
)
...
WITH eventos_limpios AS (
    SELECT
        CASE WHEN TRIM(s.raw_momentum_dx) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_momentum_dx)::FLOAT ELSE 0 END AS momentum_dx,
        TRIM(s.raw_pos_x)::FLOAT AS pos_x,
        TRIM(s.raw_pos_y)::FLOAT AS pos_y,
        CASE WHEN TRIM(s.raw_pos_z) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_pos_z)::FLOAT ELSE 0 END AS pos_z,
        CASE WHEN TRIM(s.raw_angulo_vision) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_angulo_vision)::FLOAT ELSE 0 END AS angulo_vision,
        CASE WHEN TRIM(s.raw_momentum_dy) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_momentum_dy)::FLOAT ELSE 0 END AS momentum_dy,
        CASE WHEN TRIM(s.raw_velocidad) ~ '^-?[0-9]+(\.[0-9]+)?$'
            THEN TRIM(s.raw_velocidad)::FLOAT ELSE 0 END AS velocidad_momentum,
        CASE WHEN TRIM(s.raw_salud) ~ '^[0-9]+$'
            THEN TRIM(s.raw_salud)::INTEGER ELSE 0 END AS salud,
        CASE WHEN TRIM(s.raw_armadura) ~ '^[0-9]+$'
            THEN TRIM(s.raw_armadura)::INTEGER ELSE 0 END AS armadura,
        CASE WHEN TRIM(s.raw_municion_balas) ~ '^[0-9]+$'
            THEN TRIM(s.raw_municion_balas)::INTEGER ELSE 0 END AS municion_balas,
    ...
    INNER JOIN tic t
        ON t.numero_tic = TRIM(s.raw_numero_tic)::INTEGER
       AND t.sesiones_juegosesion_id = TRIM(s.raw_sesion)::INTEGER
    WHERE s.error_flag = FALSE
),
numerados AS (
